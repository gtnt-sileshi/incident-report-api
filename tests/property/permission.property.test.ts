...
describe('Property Test: Cascading Permission Removal (P2)', () => {
  ...
  const permissionArbitrary = fc.constantFrom(...SYSTEM_PERMISSIONS).map((name) => ({
    id: SYSTEM_PERMISSIONS.indexOf(name) + 1,
    name,
    groupName: name.split('.')[0].charAt(0).toUpperCase() + name.split('.')[0].slice(1),
  }));

  const permissionSetArbitrary = fc.array(permissionArbitrary, { minLength: 1, maxLength: 20 }).map((perms) => {
  ...
  const userCountArbitrary = fc.integer({ min: 1, max: 10 });
  ...
  it('Property 2: Removing permission from org removes it from all users and maintains subset invariant', () => {
    fc.assert(
      fc.asyncProperty(
        ...
        userCountArbitrary,
        async (initialOrgPermissions, userCount) => {
          if (initialOrgPermissions.length === 0) return;
          ...
          for (let i = 0; i < userCount; i++) {
            userIds.push(`user-${i}-${Math.random().toString(36).substring(7)}`);
          }

          const permissionToRemove = initialOrgPermissions[
            Math.floor(Math.random() * initialOrgPermissions.length)
          ];

          const initialUserPermissions: Map<string, typeof initialOrgPermissions> = new Map();
          
          for (const userId of userIds) {
            const userPerms = initialOrgPermissions.filter(() => Math.random() < 0.5);
            initialUserPermissions.set(userId, userPerms);
          }

          const updatedOrgPermissions = initialOrgPermissions.filter(
            (p) => p.id !== permissionToRemove.id
          );

          const updatedUserPermissions: Map<string, typeof initialOrgPermissions> = new Map();
          for (const [userId, perms] of initialUserPermissions.entries()) {
            const updatedPerms = perms.filter((p) => p.id !== permissionToRemove.id);
            updatedUserPermissions.set(userId, updatedPerms);
          }

          (organizationRepository.getOrgPermissions as jest.Mock).mockImplementation(() => 
            Promise.resolve([...updatedOrgPermissions])
          );

          (userRepository.getUserPermissions as jest.Mock).mockImplementation((userId: string) => {
            const perms = updatedUserPermissions.get(userId) || [];
            return Promise.resolve([...perms]);
          });

          const finalOrgPerms = await organizationRepository.getOrgPermissions(orgId);
          const orgPermNames = new Set(finalOrgPerms.map((p) => p.name));
          expect(orgPermNames.has(permissionToRemove.name)).toBe(false);

          for (const userId of userIds) {
            const finalUserPerms = await userRepository.getUserPermissions(userId);
            const userPermNames = new Set(finalUserPerms.map((p) => p.name));
            expect(userPermNames.has(permissionToRemove.name)).toBe(false);
          }

          for (const userId of userIds) {
            const finalUserPerms = await userRepository.getUserPermissions(userId);
            
            for (const userPerm of finalUserPerms) {
              expect(orgPermNames.has(userPerm.name)).toBe(true);
            }
          }
        }
      ),
